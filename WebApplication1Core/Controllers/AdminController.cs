...
namespace WebApplication1Core.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> Edit(SignUpModel signUpModel, string oldRole, string oldEmail, string oldPassword)
        {
            try
            {
        ...
        public async Task<ActionResult> viewSubmissions(string coordinator, string Student)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> Delete(SignUpModel model)
        {
            client = new FireSharp.FirebaseClient(config);
            FirebaseAuthProvider auth = new(new FirebaseConfig(ApiKey));
